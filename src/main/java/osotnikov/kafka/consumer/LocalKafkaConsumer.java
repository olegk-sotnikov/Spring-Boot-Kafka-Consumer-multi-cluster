...
@Component
public class LocalKafkaConsumer implements ConsumerSeekAware {

	private static final Logger logger = LoggerFactory.getLogger(LocalKafkaConsumer.class);
	...
	@KafkaListener(id = "local-listener", groupId = "some-group", topics = "example-topic", containerFactory = "localKafkaListenerContainerFactory")
	public void receivedMessage(String message) {
		if(countdown.getAndDecrement() > 0)
			logger.info("\n\tMessage received using LOCAL Kafka listener:\n\t" + message + "\n");
		else
			registry.getListenerContainer("local-listener").pause();
	}
